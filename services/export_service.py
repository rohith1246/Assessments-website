...
def export_csv(search: str = '', assessment_id: int = None, status: str = None):
    ...
    filename = f'lionix_results{status_suffix}_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}.csv'
    return Response(
        generate(),
...
def export_xlsx(search: str = '', assessment_id: int = None, status: str = None):
    ...
    title_cell.value = f'Lionix — Assessment Results Export{status_title}  ({datetime.utcnow().strftime("%d %b %Y %H:%M UTC")})'
    ...
    status_suffix = f'_{status.lower()}' if status and status.lower() in ('pass', 'fail') else ''
    filename = f'lionix_results{status_suffix}_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}.xlsx'
...
def export_daily_reports_csv(start_date: str = '', end_date: str = '', assessment_id: int = None):
    ...
    filename = f'lionix_daily_assessment_reports_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}.csv'
    return Response(
        generate(),
        mimetype='text/csv; charset=utf-8',
        headers={'Content-Disposition': f'attachment; filename="{filename}"'}
...
def export_daily_reports_xlsx(start_date: str = '', end_date: str = '', assessment_id: int = None):
    ...
    title_cell = ws['A1']
    title_cell.value = f'Lionix — Day-by-Day Assessment Reports Export ({datetime.utcnow().strftime("%d %b %Y %H:%M UTC")})'
    title_cell.font = Font(bold=True, color='4C1D95', size=13)
    title_cell.fill = PatternFill('solid', fgColor='EDE9FE')
    title_cell.alignment = center
    ws.row_dimensions[1].height = 26
    ...
    filename = f'lionix_daily_assessment_reports_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}.xlsx'
    response = make_response(output.getvalue())
    response.headers['Content-Disposition'] = f'attachment; filename="{filename}"'
